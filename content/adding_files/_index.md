...
##### ■TIPS
下記のようにファイルではなくパイプを利用して直接「hello world」を`add`することも可能であり、またその参照ハッシュ値もファイルでの追加時と同一であることからも、`ipfs add`で追加するものは「ファイル」ではなく「コンテンツ自身」であることが実感できるでしょう。
...
$ cmp cat.jpg cat_output.jpg    #念のため中身も比較。
